...
public class CatalogDependenciesAuxiliaryMB extends CommonCatalogMB<Catdaa> implements Serializable {
	...
	private static final String FIELD_REQUIRED_CLAVE= FrontProperty.getPropertyValue("catalog.message.field.required.clave");
	
	/**  Nombre del reporte en texto plano. */
	//@Value("${file.name.report.txt.dependenciasAuxiliaresAdm}")
	protected static final String REPORT_NAME_PLAIN_TEXT= FrontProperty.getPropertyValue("file.name.report.txt.dependenciasAuxiliaresAdm");
	
	/**  Encabezados reporte de texto plano. */
	//@Value("${header.text.plain.dependencies.auxiliary}")
	protected static final String HEADERS_REPORT_TEXT_PLAIN= FrontProperty.getPropertyValue("header.text.plain.dependencies.auxiliary");
	
	/** The Constant HEADERS_REPORT_CSV. */
	protected static final String HEADERS_REPORT_CSV= FrontProperty.getPropertyValue("header.csv.dependencies.auxiliary");
	...
	/** The Constant ERROR_MESSAGE_LENGTH_CLAVE. */
	//@Value("${catalog.message.error.catdaa.clave.length}")
	private static final String ERROR_MESSAGE_LENGTH_CLAVE= FrontProperty.getPropertyValue("catalog.message.error.catdaa.clave.length");
	
	/**  Ruta donde se encuentra el archivo jasper del reporte. */
	//@Value("${view.report.path.jasper.catalog_dependencies_auxiliary}")
	private static final String REPORT_PATH_JASPER_FILE= FrontProperty.getPropertyValue("view.report.path.jasper.catalog_dependencies_auxiliary");
	...
	@PostConstruct
    public void init() {
		LOGGER.info(":: En postconsruct catalogDependenciesAuxiliaryMB " + this + "::" + getBeanFind() + "::" + getList());		
	...
	public void onRowEdit(final RowEditEvent event) {
		...
		if(validateSaveOrUpdate(catalog)) {
			...
			if(errorMsg.length() == ZERO) {
				generateNotificationFront(SEVERITY_ERROR, MESSAGE_ERROR, MESSAGE_ERROR_UPDATE_UNIQUE.concat(FIELD_REQUIRED_CLAVE).
