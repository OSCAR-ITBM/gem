...
public class AcumulacionSaldosMB extends AbstractMB{
  ...
  public void processMunicipal(Integer idSector){
    ...
    ingresoRepository.executeAcumulaIngreso(idSector, getUserDetails().getUsername());
    progmun(idSector, getUserDetails().getUsername());
    acumulaPasot(idSector,getUserDetails().getUsername());
    preprograRepository.executeAcumulaPreprogra(idSector,getUserDetails().getUsername());
    cuentaRepository.executeAcumulacionSaldos(idSector);
    salSum1(idSector.longValue());
    ingresotRepository.executeAcumulaIngresot(idSector, getUserDetails().getUsername());
    cuentaRepository.updateNameByCuentaAndSctaAndIdsector("LEY DE INGRESOS POR EJECUTAR", idSector.longValue(), "8120", "");
  ...
  private void progmun(Integer idSector, String userId){
   List<Object[]> results= pasoRepository.findClaveAndProgramaByIdSector(idSector);
    for(Object[] res:results){
      ...
      if(programamunRepository.findFirstByCvedepgAndCvedepaAndProgramaAndIdsector(cvdepg,cvdepa , (String)res[1], idSector)==null){
  ...
  private void updateDigitAccounts(Long idSector){
    ...
    results.stream().forEach((res) -> {
      ...
      findAndUpdate(idSector, account, ((BigDecimal) res[1]).doubleValue(), "NO EXISTE CUENTA DE MAYOR CON TRES CEROS");
    });
  }
   ...
   * @param message the message
   */
  private void updateAccountRangeSalini(Long idSector, String minCta, String maxCta, String searchCuenta, String scta , String message){
