...
public class CatalogDependenciesGeneralMB extends CommonCatalogMB<Catdgm> implements Serializable {
	...
	protected static final String REPORT_NAME_PLAIN_TEXT= FrontProperty.getPropertyValue("file.name.report.txt.dependenciasGenerales");
	
	/**  Encabezados reporte de texto plano. */
	//@Value("${header.text.plain.dependencies.auxiliary.general}")
	protected static final String HEADERS_REPORT_TEXT_PLAIN= FrontProperty.getPropertyValue("header.text.plain.dependencies.auxiliary.general");
	
	/** The Constant HEADERS_REPORT_CSV. */
	protected static final String HEADERS_REPORT_CSV= FrontProperty.getPropertyValue("header.csv.dependencies.auxiliary.general");
	
	/** Campo por el cual se ordenará la info. */
	private static final String ORDER_BY_FIELD_CLAVE ="clave";
	
	/** The error message length clave. */
	//@Value("${catalog.message.error.catdgm.clave.length}")
	private String ERROR_MESSAGE_LENGTH_CLAVE= FrontProperty.getPropertyValue("catalog.message.error.catdgm.clave.length");
	
	/**  Ruta donde se encuentra el archivo jasper del reporte. */
	//@Value("${view.report.path.jasper.catalog_dependencies_general}")
	private static final String REPORT_PATH_JASPER_FILE= FrontProperty.getPropertyValue("view.report.path.jasper.catalog_dependencies_general");
	...
	@PostConstruct
    public void init() {
		LOGGER.info(":: En postconsruct catalogDependenciesGeneralMB " + this + "::" + getBeanFind() + "::" + getList());								
