...
public class MovimientosDeudaMB extends BaseDirectReport {
	...
	public Map<String, Object> getParametersReports() {
		...
		parameters.put("P_L3", firmas.getL1());
		parameters.put("P_N3", firmas.getN1());
		parameters.put("P_L4", firmas.getL2());
		parameters.put("P_N4", firmas.getN2());
	...
	private void obtenerDatosCuentaInformada() {
		if(!todosVacios()){
			List<Cuenta> listaCuenta = cuentaRepository.findAllByCuentaAndNomctaAndIdsector(cuenta.getCuenta(), cuenta.getScuenta(), cuenta.getSscuenta(), cuenta.getSsscuenta(), cuenta.getSssscuenta(), null, ((Integer) getUserDetails().getIdSector()).longValue());
	...
	private boolean validateCuenta(){
		...
		if(!isValid){
			...
			List<Cuenta> lista = cuentaRepository.findAllByCuentaAndNomctaAndIdsector(cuenta.getCuenta(), cuenta.getScuenta(),
