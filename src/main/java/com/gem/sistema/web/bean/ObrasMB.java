...
@SessionScoped
public class ObrasMB extends AbstractMB implements Serializable {
					...
					if(BigDecimal.ZERO.compareTo(obra.getAuto13())<0) {
						...
						if(BigDecimal.ZERO.compareTo(autoi13)!= 0) {
							...
							if (messageOk.equals(validacion)) {
								...
								context.addMessage(null,new FacesMessage(FacesMessage.SEVERITY_INFO ,"Info", "Programa de Obra Anual guardada correctamente"));
							} else {
								FacesContext context = FacesContext.getCurrentInstance();
							...
							generateNotificationFront(FacesMessage.SEVERITY_ERROR, "Error!", "No hay suficiente presupuesto");
							buttonsBoolean();
						}
					} else {
						generateNotificationFront(FacesMessage.SEVERITY_ERROR, "Error!", "La suma del presupuesto calendarizado de egresos debe de ser mayor a cero");
						buttonsBoolean();
						
						
					}
				} else {
					generateNotificationFront(FacesMessage.SEVERITY_ERROR, "Error!", "La partida no tiene presupuesto (6xxx)");
					buttonsBoolean();
				}
			} else {
				buttonsBoolean();
	...
	public boolean validaCampos(final Obra obra) {
		...
		if(StringUtils.isEmpty(obra.getFn())) {
			...
			generateNotificationFront(FacesMessage.SEVERITY_ERROR, "Error!", "Campo SubProg debe de ser de dos Números");
			bandera  = Boolean.FALSE;
	...
	public void borrar(Integer row) {
			...
			try {
				...
				UtilFront.generateNotificationFront(FacesMessage.SEVERITY_INFO,
						"Info!", "Programa de Obra Anual borrada correctamente");
			} catch (Exception e) {
			...
			try {
				...
				if (!obrasCargaAutomaticaDTO.getErrorMsg().isEmpty()) {
					if(obrasCargaAutomaticaDTO.getErrorMsg().startsWith("1")){
						FacesMessage message = new FacesMessage("Es posible que el registro del archivo de carga automatica de Obras ya haya sido procesado anteriormente");
						FacesContext.getCurrentInstance().addMessage(null, message);
					}else{
	...
	/**
	 * Busca por estructura programática/No. control/Nombre de Obra y actualiza
	 * la lista de obras con el resultado
	 */
