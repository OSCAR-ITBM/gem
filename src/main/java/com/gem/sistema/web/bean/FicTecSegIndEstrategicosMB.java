...
/**
 * Managed Bean para el control del módulo Ficha tecnica para el seguimiento de indicadores de gestión o estrategicos.
 *
 * @author BIITEC
 */
...
public class FicTecSegIndEstrategicosMB extends AbstractMB implements Serializable {
	...
	/**
	 * realiza una consulta en la base de datos para obtener los indicadores a mostrar.
	 *
	 * @param row the row
	 */
	...
	/**
	 * Inicializa los componentes de la sección encabezado para un objeto ficha tecnica dado.
	 *
	...
	/**
	 * Inicailiza los componentes de la sección detalles para una configuración de ficha tecnica dada.
	 *
	...
	private void cargarCatalogos() {
		...
		//		List<Xcatpro> dependencias =  xcatproRepository.findCustomDistinct(getUserDetails().getIdSector());
	...
	/**
	 * Inicializa las banderas que definen comportamiento visual de los componentes de la pagina.
	 */
	...
	/**
	 * Modifica las banderas para habilitar/deshabilitar el modo de edición del encabezado.
	 *
	 * @param edit Cuando es <code>true</code>, habilita la edición, en caso contrario la deshabilita
	 */
	...
	/**
	 * Modifica las banderas para habilitar/deshabilitar el modo de edición del detalle.
	 *
	 * @param edit Cuando es <code>true</code>, habilita la edición, en caso contrario la deshabilita
	 */
	...
	/**
	 * Persiste en la base de datos los cambios realizados en el encabezado de la ficha tecnica seleccionada.
	 *
	...
	/**
	 * Verifica que se cumplan las reglas de negocio definidas para el encabezado de la ficha tecnica.
	 *
	 * @param ft Ficha tecnica a validar
	 * @return <code>true</code> si todas las reglas se cumplen; <code>false</code> en caso contrario
	 */
	private boolean validarEncabezado(Ftecnicasm ft) {
		...
		List<Ftecnicasm> existentes = ftecnicasmRepository.findByCvetemasAndClvdepAndClvfunAndCveind(
				ft.getCvetemas(), ft.getClvdep(), ft.getClvfun(), ft.getCveind());
	...
	/**
	 * Agrega una Ficha Tecnica nueva en pantalla para ser configurarda y posteriormente registrada en la base.
	 */
	...
	/**
	 * Elimina fIsicamente de la base de datos el registro asociado a la ficha tecnica seleccionada.
	 *
	 * @param row Inidice de la Ficha tecnica seleccionada
	 */
	public void borrarEncabezado(Integer row) {
		...
		try {
			...
			setBanderasEditEncabezado(false);
			irAPaginaEncabezado(row);
			generateNotificationFront(FacesMessage.SEVERITY_INFO, "El registro fue eliminado correctamente", "Borrar Encabezado");
		} catch (Exception e) {
			e.printStackTrace();
			generateNotificationFront(SEVERITY_ERROR, "Ocurrió un error al eliminar el registro: " + e.getMessage(), "Data Error");
		}
	}

	/**
	 * Cancela la edicipon y restaura los valores persistidos para la ficha tecnica selccionada.
	 * En caso de ser una ficha nueva que todavIa no existe en base de datos, la elimina de la lista.
	 * @param row Inidice de la Ficha tecnica seleccionada
	 */
	...
	/**
	 * Persiste en la base de datos los cambios realizados en el detalle de la ficha tecnica seleccionada.
	 *
	...
	public void salvarDetalle(Integer row) {
		...
		if (validarDetalle(ftd)) {
			try {
				...
				if (ftd.getCodigo() == 0) {
					int mayor = 0;
					List<Ftecnicasd> regs = ftecnicasdRepository.findByCvetemasAndClvdepAndClvfunAndCveind(ftd.getCvetemas(), ftd.getClvdep(), ftd.getClvfun(), ftd.getCveind());
				...
				generateNotificationFront(FacesMessage.SEVERITY_INFO, "La informacion se guardó correctamente", "Salvar Detalle");
			} catch (Exception e) {
				e.printStackTrace();
				generateNotificationFront(SEVERITY_ERROR, "Ocurrió un error al guardar la información: " + e.getMessage(), "Data Error");
			}
	...
	/**
	 * Verifica que se cumplan las reglas de negocio definidas para el detalle de la ficha tecnica.
	 *
	 * @param ftd the ftd
	 * @return <code>true</code> si todas las reglas se cumplen; <code>false</code> en caso contrario
	 */
	private boolean validarDetalle(Ftecnicasd ftd) {
		...
		List<Ftecnicasd> existentes = ftecnicasdRepository.findByCvetemasAndClvdepAndClvfunAndCveindAndCodigo(
				temaOrig, dependenciaOrig, programaOrig, codIndOrig, ftd.getCodigo());
		...
		if (duplicado) {
			String msg = "Ftecnicasd ya existe con Cve Temas \"" + ftd.getCvetemas() + "\" "
					+ "clave  Dependencia \"" + ftd.getClvdep() + "\" "
					+ "Programa \"" + ftd.getClvfun() + "\" "
					+ "Fuente de Financiamiento \"" + ftd.getClvfin() +"\" "
					+ "Codigo de Indicador \"" + ftd.getCveind() + "\" "
					+ "Código \"" + ftd.getCodigo() + "\" (132) Update cancelled.";
			generateNotificationFront(SEVERITY_ERROR, msg, "Data Error");
			return false;
		}
		return true;
	...
	/**
	 * Handler para evaluar los cambios en el campo código de variable. 
	 * Al cambiar el valor del campo se verifica que el código exista en la tabla variables; 
	 * en caso de no existir se manda un mensaje de error 
	 * @param row Inidice del detalle de la Ficha tecnica seleccionada
	 */
	public void cambiarVariable(Integer row) {
		Ftecnicasd ftd = detallesList.get(row);
		if (StringUtils.isEmpty(ftd.getCvevar())) {
			ftd.setCvevar(StringUtils.EMPTY);
			ftd.setVariables(StringUtils.EMPTY);
			return;
		}
		ftd.setCvevar(ftd.getCvevar().toUpperCase());
		List<Variables> variables = variablesRepository.findByCvevar(ftd.getCvevar().toUpperCase());
		if (variables  == null || variables.isEmpty()) {
			...
			ftd.setVariables(variables.get(0).getNomvar());
		}
	}
	...
	/**
	 * Agrega un detalle de Ficha Tecnica en pantalla para ser configurardo y posteriormente registrado en la base.
	 */
	...
	/**
	 * Elimina fIsicamente de la base de datos el registro asociado al detalle de la ficha tecnica seleccionada.
	 *
	 * @param row Inidice del detalle de la Ficha tecnica seleccionada
	 */
	public void borrarDetalle(Integer row) {
		...
		try {
			...
			setBanderasEditDetalle(false);
			irAPaginaDetalle(row);
			generateNotificationFront(FacesMessage.SEVERITY_INFO, "El registro fue eliminado correctamente", "Borrar Detalle");
		} catch (Exception e) {
			e.printStackTrace();
			generateNotificationFront(SEVERITY_ERROR, "Ocurrió un error al eliminar el registro: " + e.getMessage(), "Data Error");
		}
	...
	/**
	 * Cancela la edición y restaura los valores persistidos para el detalle de la ficha tecnica selccionada.
	 * En caso de ser un detalle nuevo que todavIa no existe en base de datos, lo elimina de la lista.
	 * @param row Inidice del detalle de la Ficha tecnica seleccionada
	 */
...
//		}
	...
	public void changePrograma(Integer row) {
		...
		List<Muninep> progs = muniNepRepository.findByCampo7AndIdsector(ftecnicasm.getClvfun(), getUserDetails().getIdSector());
		if (progs != null && !progs.isEmpty()) {
	...
	public void changeCodInd(Integer row) {
		...
		if (codInds != null && !codInds.isEmpty()) {
			...
			desCodInd = desIdCodInd = StringUtils.EMPTY;
			generateNotificationFront(SEVERITY_ERROR, "No se encontró información relacionada al indicador.", "Data Error");
		}
	}
	...
	private void restartFilteredVariables() {
		if (BooleanUtils.negate(filteredVariables == null)
				&& BooleanUtils.negate(variables == null)) {
	...
	private void restartFilteredIndicadores() {
		if (BooleanUtils.negate(filteredIndicadores == null)
				&& BooleanUtils.negate(indicadores == null)) {
...
//		context.update("formIndCaptura:panelIndicadores:indDetalleTable:" + detalleRowSelected + ":varDescripcion");
	...
	public boolean isHabilitarBorrarMaestro() {
		return capturaDeshabilitado && !(encabezadosList == null || encabezadosList.isEmpty() || encabezadosList.get(0).getId() == null || encabezadosList.get(0).getId() == 0);
	}
