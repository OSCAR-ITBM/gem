...
//		currentPageCaptura = 0;
	...
	private void cargarDatos() {
		...
		if(masterList.get(currentPageCaptura).getDetalle()==null || masterList.get(currentPageCaptura).getDetalle().size()==0
	...
	private void inicializarCatalogos(){
		...
		for(Xcatpro xcat : metasFisicasService.listaDependencias(sector)){
			if(!clvDependenciasList.contains(xcat.getClvdep())) clvDependenciasList.add(xcat.getClvdep());
		}		
	}
	...
	//busca la lista de programas de esa dependencia y su descripcion
		public void filtraPrograma(Integer row){
			...
			for(Xcatpro dep:listaPro){
				if(dep.getSectorid().equals(this.getUserDetails().getIdSector())){
					if(!clvProgramaList.contains(dep.getClvfun())) clvProgramaList.add(dep.getClvfun());
				}
			}
		...
		public void filtraFuente(Integer row){
			LOGGER.info("*** Filtrando fuentes " + masterList.get(row).getClvDep() + " - " + masterList.get(row).getClvnep());
			clvFuenteList.clear();
			List<Xcatpro> listaFuen = metasFisicasService.filtraFuente(masterList.get(row).getClvDep(),masterList.get(row).getClvnep());
		...
		public void filtraFuenteDesc(Integer row){				
			masterList.get(row).setDesfuen(metasFisicasService.llenaDescFuente(masterList.get(row).getClvfuen(), this.getUserDetails().getIdSector()));
		}
	...
	private void estadoInicialBotonesMeta() {
		if(masterList.get(currentPageCaptura).getDetalle()==null || masterList.get(currentPageCaptura).getDetalle().size()==0
				|| masterList.get(currentPageCaptura).getDetalle().get(currentPageMeta).getClvmet()==null){//no tiene detalle
	...
	public void modificarCaptura(Integer row) {
		...
		if(masterList.get(row).getDetalle().size() > 0 && masterList.get(row).getDetalle().get(0).getId()!=null){//si tiene detalles no puede modificar
			this.setDeshabilitadoEditables(false);
			this.setCapturaDeshabilitado(true);
		}else{
			this.setDeshabilitadoEditables(false);
	...
	public void salvarCaptura(Integer row) {
		...
		if(masterList.get(row).getClvDep() == null || masterList.get(row).getClvnep() == null || masterList.get(row).getClvfuen() == null){
			UtilFront.generateNotificationFront(FacesMessage.SEVERITY_ERROR, "Los campos Dependencia Ejecutora, FN/Fun/SubF/Prog/SubProg/Proy y Fuente de Financiamiento son requeridos", "Los campos Dependencia Ejecutora, FN/Fun/SubF/Prog/SubProg/Proy y Fuente de Financiamiento son requeridos");
			return;
		}
			...
			MetasFisicasDTO indcap = new MetasFisicasDTO();
				...
				if(masterList.get(row).getId()!=null){
					...
					MetasFisicasDTO det = metasFisicasService.findByClvdepClvNepClvFuen(masterList.get(row).getClvDep(), 
							...
							masterList.get(row).getClvfuen());
					if(det != null){
						UtilFront.generateNotificationFront(FacesMessage.SEVERITY_ERROR, "Ya existe en encabezado seleccionado",
								"Ya existe en encabezado seleccionado");
						return;
					}
				}
...
//		temp = metasFisicasService.findByClvdepClvNepClvFuen(temp.getClvDep(), temp.getClvnep(), temp.getClvfuen());//para saber si esta en la base
		if (capturaAdicionar) {
			...
			if(metaListOri.get(row).getDetalle()==null || metaListOri.get(row).getDetalle().size()==0){//no tiene detalle
				List<MetasFisicasDetalleDTO> detFake = new ArrayList<MetasFisicasDetalleDTO>();
				MetasFisicasDetalleDTO d = new MetasFisicasDetalleDTO();
				d.setClvmet(null);
				detFake.add(d);
...
//		}
	...
	public void borrarCaptura(Integer row) {
		try{
			if(masterList.get(row).getDetalle().size() > 0 && masterList.get(row).getDetalle().get(0).getId()!=null){
				UtilFront.generateNotificationFront(FacesMessage.SEVERITY_ERROR, "Existen Codigos, no se puede eliminar", "Existen Codigos, no se puede eliminar");
			}else{
			...
			UtilFront.generateNotificationFront(FacesMessage.SEVERITY_ERROR, "Ocurrió un error al borrar el Encabezado ",
					"Ocurrió un error al borrar el Encabezado ");
		}	
	}
...
//		this.estadoInicialBotones();
	...
	public void adicionarMeta(Integer rowCaptura) {
		...
		if(masterList.get(rowCaptura).getDetalle().size() == 1){
			if(masterList.get(rowCaptura).getDetalle().get(0).getId() == null || masterList.get(rowCaptura).getDetalle().get(0).getId().equals(0)){
	...
	public void salvarMeta(Integer rowCaptura, Integer rowDetail) {//mandar row del cabecero y de la meta
		...
		try {
			MetasFisicasDetalleDTO det = metasFisicasService.findByClvdepClvNepClvFuenClvMet(masterList.get(rowCaptura).getClvDep(), 
					masterList.get(rowCaptura).getClvnep(), 
...
//		this.estadoInicialBotones();
		...
		if(masterList.get(rowCaptura).getDetalle().size() == 1 && masterList.get(rowCaptura).getDetalle().get(rowDetail).getId() == null){
			masterList.get(rowCaptura).getDetalle().clear();
			masterList.get(rowCaptura).getDetalle().add(new MetasFisicasDetalleDTO());
		}else{
			if(metaAdicionar){
//				masterList.get(rowCaptura).getDetalle().remove(masterList.get(rowCaptura).getDetalle().size()-1);
				masterList.get(rowCaptura).getDetalle().set(masterList.get(rowCaptura).getDetalle().size()-1, new MetasFisicasDetalleDTO());
			}else{
				MetasFisicasDetalleDTO det = metasFisicasService.findByClvdepClvNepClvFuenClvMet(masterList.get(rowCaptura).getClvDep(), 
						masterList.get(rowCaptura).getClvnep(), 
						masterList.get(rowCaptura).getClvfuen(), 
	...
	public void borrarMeta(Integer rowCaptura, Integer rowDetail) {
		try{
			metasFisicasService.deleteMeta((masterList.get(rowCaptura).getDetalle().get(rowDetail).getId().longValue()));
...
//						masterList.get(rowCaptura).getClvfuen(),
//						masterList.get(rowCaptura).getDetalle().get(rowDetail).getClvmet());
				masterList.get(currentPageCaptura).getDetalle().remove(masterList.get(currentPageCaptura).getDetalle().size() - 1);
				masterList.get(currentPageCaptura).getDetalle().set(rowDetail-1, masterList.get(currentPageCaptura).getDetalle().get(rowDetail-1));
	...
	public void cambiarPaginaMeta(PageEvent event) {
		...
		masterList = metasFisicasService.getEncabezados(this.getUserDetails().getIdSector());
		if(masterList.get(currentPageCaptura).getDetalle()==null || masterList.get(currentPageCaptura).getDetalle().size()==0){//no tiene detalle
			this.setMetaVisibleModificar(false);
			this.setMetaVisibleSalvar(false);
			this.setMetaDesabilitadoReset(true);
			this.setMetaDesabilitadoAdicionar(false);
