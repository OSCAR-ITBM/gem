...
public interface CuentaRepository
	...
	/**
	 * Find one by cuenta and scuenta and sscuenta and ssscuenta and sssscuenta and idsector.
	 *
	 * @param cuenta the cuenta
	...
	@Query(value = "  select c.* " + " from Cuenta c "
			+ " where (:cuenta = '' or :cuenta is null or c.cuenta = :cuenta) " + " and c.idsector = :idSector "
			+ " and (:nomcta = '' or :nomcta is null or upper(c.nomcta) like '%' || :nomcta || '%') "
			+ " and ((:scta = '' or :scta is null or c.scta = :scta) "
			+ " and (:sscta = '' or :sscta is null or c.sscta = :sscta) "
			+ " and (:ssscta = '' or :ssscta is null or c.ssscta = :ssscta) "
			+ " and (:sssscta = '' or :sssscta is null or c.sssscta = :sssscta) )"
			+ " order by c.cuenta", nativeQuery = true)
	List<Cuenta> findAllByCuentaAndNomctaAndIdsector(@Param("cuenta") String cuenta, @Param("scta") String scta,
			...
			@Param("nomcta") String nomcta, @Param("idSector") Long idSector);
  ...
  List<Cuenta> findAllWhereCuentaBeginsAndSctaBeginsAndIdSector(@Param("cta") String cuenta, @Param("scta")String scta,  @Param("idsector") Long idSector);
  ...
  Cuenta findFirstByCuentaAndIdsectorOrderByIdAsc(@Param("cta") String cuenta, @Param("idsector") Long idsector);
	...
	int updateNameByCuentaAndSctaAndIdsector(@Param("nomcta") String nomcta, @Param("idsector") Long idsector, @Param("cuenta") String cuenta, @Param("scta") String scta);
